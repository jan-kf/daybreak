...
class Player(Entity):
    ...
    def shoot(self):
        ...
        if now - self.last_shot > WEAPONS[self.weapon]['rate']:
            ...
            pos = self.pos# + (BARREL_OFFSET*self.game.zoom.scale_factor).rotate(-self.rot)
            print(pos)
            self.vel = vec(-(self.game.zoom.get_linear_update(WEAPONS[self.weapon]['kickback'])), 0).rotate(-self.rot)
            for i in range(WEAPONS[self.weapon]['bullet_count']):
                _spread = self.game.zoom.get_linear_update(WEAPONS[self.weapon]['spread'])
...
class Mob(Entity):
    ...
    def update(self):
        ...
        if target_dist.length_squared() < self.game.zoom.get_linear_update(DETECT_RADIUS)**2:
            if random() < 0.002:
...
class Bullet(Entity):
    def __init__(self, game, pos, dir, damage):
        ...
        self.vel = dir * self.game.zoom.get_linear_update(WEAPONS[game.player.weapon]['bullet_speed']) * uniform(0.9, 1.1)
